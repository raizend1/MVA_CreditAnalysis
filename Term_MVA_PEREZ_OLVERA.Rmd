...
```{r fig1,fig.width=18,fig.height=18,message=FALSE,fig.cap="Variable distribution", include=TRUE,echo=TRUE}
#***************************************************************************#
...
```{r, include=FALSE}
# We could apply a log modulus transformation in an attempt to normalize data and then plot
...
```{r Local Oulier Factor, eval=FALSE, include=FALSE}
#***************************************************************************#
...
```{r, fig2,fig.width=8,fig.height=4,message=FALSE,fig.cap="Caption", include=TRUE,echo=TRUE}
...
corrplot(cor(credit.continuos.Moutlier), method="color")
corrplot(cor(credit.continuos.Moutlier), method="shade")
mosthighlycorrelated(credit.continuos.Moutlier,10)
mosthighlycorrelated(credit.continuos.Moutlier,10)
...
#describeBy(credit.continuos.Moutlier, credit$default.payment.next.month)
#pairs(credit.continuos.Moutlier, main = "Default payment pair plot", col = (1:length(levels(credit$default.payment.next.month)))[unclass(credit$default.payment.next.month)])
```
...
require(FactoMineR)
par(mfrow=c(2,2))
credit.PCA1 <- PCA(credit.continuos)
credit.PCA2 <- PCA(credit.continuos.log)
```
