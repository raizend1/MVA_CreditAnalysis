...
create.latex.table(df=print(credit.tree.leaf),type = "latex",caption = "Leaves information",file = "leaves_information.tex")
...
```{r , echo=FALSE}
# for error Error in approxfun(x.values.1, y.values.1, method = "constant", f = 1, : zero non-NA points
require(dplyr)
credit[credit.test,]%>% count(default.payment.next.month)
# resort dataframe
set.seed(555) 
credit.test <- sample(credit.test)

credit.sp[credit.test,]%>% count(default.payment.next.month)
```
...
pred <- prediction(pred.test.df$Default, credit.sp$default.payment.next.month[credit.test])
roc <- performance(pred,measure="tpr",x.measure="fpr")
plot(roc, main="ROC curve")
abline(0,1,col="blue")
library(pROC)
auc(roc(credit.sp[credit.test,]$default.payment.next.month, pred.test.df$Default))
