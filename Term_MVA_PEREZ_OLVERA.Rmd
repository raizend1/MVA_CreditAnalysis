...
```{r Check missing values output, include=FALSE,comment=NA}
...
create.latex.table(df=cbind(freq.table, p.table),type="latex",caption="Y and Sex frequencies",file=glue(dataDir,"/y_sex.tex"))
...
create.latex.table(df=cbind(freq.table, p.table),type="latex",caption="Y and Education frequencies",file=glue(dataDir,"/y_education.tex"))
...
create.latex.table(df=cbind(freq.table, p.table),type="latex",caption="Y and Marriage frequencies",file=glue(dataDir,"/y_marriage.tex"))
...
create.latex.table(df=head(arrange(age.df,desc(age.df$`Not default`)), n = 5),type="latex",caption="Y and Age Top 5 Not Default frequencies",file=glue(dataDir,"/y_age_not_default.tex"))

create.latex.table(df=head(arrange(age.df,desc(age.df$Default)), n = 5),type="latex",caption="Y and Age Top 5 Default frequencies",file=glue(dataDir,"/y_age_default.tex"))
```
...
save.plot(plot = plot(eigX$value,type="l",main="Screeplot"),name = "Screeplot.jpeg",type = "jpeg",plotDir = plotDir,height = "600",width = "600",res = "150" )
...
credit.PCA <- PCA(credit,quali.sup = factor.indexes,quanti.sup = c(18:23) ,ncp=3)
summary(credit.PCA)
```
...
credit.PCA$eig
plot(credit.PCA$eig$eigenvalue,type="l")
...
desc.credit.pca<-dimdesc(credit.PCA)
```

```{r, echo=FALSE}
setwd(dataDir)
for(i in 1:length(desc.credit.pca)){
  dim.name<-c("quanti","quali","category")
  # just take top 5 and bottom 5 of category
  eval(parse(text = glue("category<-rbind(head(as.data.frame(desc.credit.pca$Dim.",i,"$category), n = 5),tail(as.data.frame(desc.credit.pca$Dim.",i,"$category), n = 5))")))
  eval(parse(text = glue("desc.df<-cbindPad(as.data.frame(desc.credit.pca$Dim.",i,"$quanti),as.data.frame(rownames(desc.credit.pca$Dim.",i,"$quali)),as.data.frame(desc.credit.pca$Dim.",i,"$quali),as.data.frame(rownames(category)),category)")))
  eval(parse(text = glue("create.latex.table(df=desc.df,type=\"latex\",caption=\"Dimension ",i," description of dimensions\",file=\"dim",i,"_dimdesc_pca.tex\")")))
}
setwd(codeDir)
#store separate tables
# for(i in 1:length(desc.credit.pca)){
#   for(j in 1:3){
#     eval(parse(text = glue("create.latex.table(df=as.data.frame(desc.credit.pca$Dim.",i,"$",dim.name[j],"),type=\"latex\",caption=\"Dim ",i, dim.name[j],"\",file=glue(dataDir,\"/dim",i,"_",dim.name[j],".tex\"))")))
#   }
# }
...
for(i in 1:length(desc.credit.mca)){
  ...
  eval(parse(text = glue("create.latex.table(df=desc.df,type=\"latex\",caption=\"Dimension ",i," description of dimensions\",file=\"dim",i,"_dimdesc_mca.tex\")")))
}
setwd(codeDir)
...
catdes(cbind(as.factor(km$cluster), credit), num.var = 1, proba = 0.05, row.w = NULL)
```

We have generated 5 classes according the inspection that we have done in the data. Let's talk about each class and
what values should have the individual to pertain in this class. The classes are labeled in the same order that in the previous plot:
...
** Categorical Variables**
  ...
  - Around 75-85% of individuals have values of PAY_0, PAY_1, PAY_2, PAY_3, PAY_4, PAY_5 and PAY_6  equal to "URC". 
    Around 14-16% of individuals have values of PAY0, PAY_1, PAY_2, PAY_3 and PAY_4  equal to "PD2". 
...
# first to speed up things, we take an 10% stratified random sample, from all the groups in default.payment.next.month
credit.stratified<-stratified(credit, "default.payment.next.month", 0.1)
...
axis(side = 1,las=2,labels = names(Labels), at = 1:ncol(credit.importance$ImpHistory), cex.axis = 0.7)
```
...
credit.balanced<-credit[subsample.index,]

table(credit.balanced$default.payment.next.month)

train.indexes <- sample(nrow(credit.balanced), size = ceiling(length(subsample.index)*0.7))

credit.train <- as.numeric(unlist(row.names(credit.balanced[train.indexes,])))
credit.test <- as.numeric(unlist(row.names(credit.balanced[-train.indexes,])))
...
credit.tree <- rpart(default.payment.next.month ~ ., data=credit[credit.train,], control=rpart.control(cp=0.01, xval=10))
...
pred.test <- predict(p1, newdata=credit[credit.test,],type="class")

invisible(library(caret))
confusionMatrix(pred.test, credit[credit.test,]$default.payment.next.month,mode = "prec_recall")
```
...
# 
# library(caret)
# confusionMatrix(pred.test.rf, credit[credit.test,]$Adjusted,mode = "prec_recall")
```
