...
```{r, echo=FALSE}
#***************************************************************************#
#         2.5 Detection of most correlated continuous variables             #
#***************************************************************************#
...
freq.table <- (with(data = credit.Moutlier, table(SEX, default.payment.next.month)))
...
initial.barplot(credit.Moutlier,EDUCATION)
...
grouped.count.plot(credit.Moutlier,EDUCATION,default.payment.next.month)
...
freq.table <- table(credit.Moutlier$EDUCATION, credit.Moutlier$default.payment.next.month)
p.table <- round(prop.table(freq.table, margin = 1), digits = 3)
cbind(freq.table, p.table)
...
# a count of all the values to get an initial idea
initial.barplot(credit.Moutlier,AGE)

require(plyr)
head(arrange(as.data.frame(table(credit.Moutlier$AGE)),desc(Freq)), n = 5)
# from this analysis, it is obvious that the quantity of users of credit cards, are centered around
# 29 years old

# Again a check of the proportions will be useful
freq.table <- table(credit.Moutlier$AGE, credit.Moutlier$default.payment.next.month)
p.table <- round(prop.table(freq.table, margin = 1), digits = 3)
...
# credit.PCA2 <- PCA(credit.continuos,,quali.sup = c(which(names(credit.continuos.Moutlier)==c("default.payment.next.month"))),ncp = e_ncp$ncp)
# summary(credit.PCA2)
# ```
