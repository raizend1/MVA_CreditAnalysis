...
```{r Check missing values output, echo=FALSE}
...
```{r fig1,fig.width=18,fig.height=18,message=TRUE,fig.cap="Variable distribution", include=FALSE}
...
# first to speed up things, we take an 30% stratified random sample, from all the groups in default.payment.next.month
credit.stratified<-stratified(credit, "default.payment.next.month", 0.3)
cl <- makeCluster(detectCores())
registerDoParallel(cl)
...
credit.train <- credit.balanced[train.indexes,]
credit.test <- credit.balanced[-train.indexes,]
...
# Checking PCA
credit.glm<-glm((as.numeric(default.payment.next.month)-1) ~ ., data=credit.train[,-c(2,3,4,6,7,8,9,10,11)])
...
credit.tree <- rpart(Adjusted ~ ., data=credit[credit.train,], control=rpart.control(cp=0.01, xval=10))
...
lines(credit.tree$cptable[,2],audit.tree$cptable[,4],col="blue")
...
```{r Desicion tree - Prunning, echo=FALSE}
...
credit.rf <- randomForest(default.payment.next.month ~ ., data=credit[credit.train,], mtry=3, importance=TRUE, xtest=credit[-credit.train,-10], ytest=credit[-learn,10], nodesize=50, maxnodes=14, keep.forest=TRUE)
summary(credit.rf)
credit.rf$confusion
